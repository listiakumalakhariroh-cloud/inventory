    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div class="flex flex-col space-y-6 lg:col-span-1">
            ...
            <div class="hud-panel delay-200 bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col h-full hover:shadow-md transition-shadow">
                ...
                <div class="p-4 flex flex-col items-center justify-center flex-grow"><div class="relative w-full max-w-[180px] aspect-square"><canvas id="statusChart"></canvas></div></div>
        ...
        <div class="flex flex-col lg:col-span-2 hud-panel delay-300">
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 h-full flex flex-col hover:shadow-md transition-shadow">
                ...
                <div class="bg-gray-50 px-4 py-3 border-t border-gray-100 flex justify-between items-center rounded-b-xl">
                    ...
                    <div class="flex space-x-4 text-[10px] font-medium text-gray-600">
                        <div class="flex items-center"><span class="w-2 h-2 rounded-full bg-blue-600 mr-1"></span> Berjalan</div>
            ...
            <div class="hud-panel delay-500 bg-white rounded-xl shadow-sm border border-red-200 flex flex-col flex-grow hover:shadow-md transition-shadow">
                ...
                <div class="p-0 divide-y divide-gray-100 flex-grow">
                    @forelse($urgentTugas ?? [] as $penugasan)
                        @php $sisaHari = \Carbon\Carbon::parse($penugasan->batas_waktu_lapor)->startOfDay()->diffInDays(now()->startOfDay(), false) * -1; @endphp
                        <a href="{{ url('admin/penugasan/'.($penugasan->id ?? '')) }}" class="block p-3 {{ $sisaHari < 0 ? 'hover:bg-red-50' : 'hover:bg-yellow-50' }} transition cursor-pointer">
                            <p class="text-sm font-semibold text-gray-800 truncate">{{ $penugasan->tugas->nama_tugas ?? 'Data Dihapus' }}</p>
                            <div class="flex justify-between items-center mt-1"><p class="text-[10px] font-bold {{ $sisaHari < 0 ? 'text-red-600' : 'text-yellow-600' }}">{{ $sisaHari < 0 ? 'TERLAMBAT '.floor(abs($sisaHari)).' HARI' : ($sisaHari == 0 ? 'BATAS HARI INI' : 'SISA '.floor($sisaHari).' HARI') }}</p></div>
                        </a>
                    @empty
            ...
            <div class="hud-panel delay-600 bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col hover:shadow-md transition-shadow">
                ...
                <div class="p-0 divide-y divide-gray-100">
                    @forelse($laporanBaru ?? [] as $laporan)
                        <a href="{{ url('admin/manajemen-laporan/'.($laporan->id ?? '')) }}" class="block p-3 hover:bg-gray-50 transition cursor-pointer group">
                            <p class="text-sm font-semibold text-blue-600 group-hover:text-blue-800 truncate">{{ $laporan->tugas->nama_tugas ?? $laporan->penugasan->tugas->nama_tugas ?? 'Laporan Masuk' }}</p>
                            <p class="text-[10px] text-gray-500 mt-1">{{ explode(' ', $laporan->user->name ?? 'Pegawai')[0] }} • {{ $laporan->created_at->format('H:i') }} WIB</p>
                        </a>
                    @empty
...
<div id="modalStats" class="fixed inset-0 bg-black/80 z-[60] hidden flex items-center justify-center transition-opacity p-4">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col">
        <div class="bg-black px-6 py-4 flex justify-between items-center"><h3 class="text-white font-bold flex items-center"><svg class="w-5 h-5 text-yellow-400 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path></svg> Detail Statistik</h3><button onclick="closeModal('modalStats')" class="text-gray-400 hover:text-white"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12"></path></svg></button></div>
        <div class="p-6">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                ...
                <div class="bg-yellow-50 border border-yellow-200 p-4 rounded-xl"><p class="text-4xl font-black text-yellow-700">{{ $menungguReview ?? 0 }}</p><p class="text-xs font-bold text-yellow-600 mt-2">REVIEW</p></div>
                <div class="bg-green-50 border border-green-200 p-4 rounded-xl"><p class="text-4xl font-black text-green-700">{{ $selesaiBulanIni ?? 0 }}</p><p class="text-xs font-bold text-green-600 mt-2">SELESAI</p></div>
            </div>
        </div>
    </div>
</div>
...
<div id="modalStatus" class="fixed inset-0 bg-black/80 z-[60] hidden flex items-center justify-center transition-opacity p-4">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col">
        <div class="bg-black px-6 py-4 flex justify-between items-center"><h3 class="text-white font-bold flex items-center"><svg class="w-5 h-5 text-blue-400 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 3.055A9.001 9.001 0 1020.945 13H11V3.055z"></path></svg> Status Penugasan</h3><button onclick="closeModal('modalStatus')" class="text-gray-400 hover:text-white"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12"></path></svg></button></div>
        <div class="p-6 h-80 flex items-center justify-center"><canvas id="statusChartModal"></canvas></div>
    </div>
</div>
...
<div id="modalTimeline" class="fixed inset-0 bg-black/80 z-[60] hidden flex items-center justify-center transition-opacity p-4">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-5xl overflow-hidden flex flex-col h-[85vh]">
        <div class="bg-black px-6 py-4 flex justify-between items-center">
            <h3 class="text-white font-bold flex items-center"><svg class="w-5 h-5 text-yellow-400 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg> Kalender Lengkap</h3>
            <div class="flex items-center space-x-2">
                <button onclick="changeMonthModal(-1)" class="p-1 text-gray-400 hover:text-white"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M15 19l-7-7 7-7"></path></svg></button>
                <span id="modalMonthYearLabel" class="text-white text-sm font-bold font-mono min-w-[120px] text-center">Bulan</span>
                <button onclick="changeMonthModal(1)" class="p-1 text-gray-400 hover:text-white"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7"></path></svg></button>
                <button onclick="goToTodayModal()" class="ml-2 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded shadow">HARI INI</button>
                <button onclick="closeModal('modalTimeline')" class="text-gray-400 hover:text-white ml-4"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12"></path></svg></button>
            </div>
        </div>
        
        <div class="p-6 flex flex-col flex-grow overflow-hidden bg-gray-50/30">
            <div class="flex items-center mb-6 relative">
                <button onclick="scrollDates('modalDateScrollContainer', -300)" class="p-3 rounded-lg bg-gray-100 hover:bg-gray-200 absolute left-0 z-10 shadow-sm border border-gray-200"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M15 19l-7-7 7-7"></path></svg></button>
                <div id="modalDateScrollContainer" class="drag-scroll flex overflow-x-auto scrollbar-hide space-x-3 px-16 flex-grow cursor-grab select-none py-2"></div>
                <button onclick="scrollDates('modalDateScrollContainer', 300)" class="p-3 rounded-lg bg-gray-100 hover:bg-gray-200 absolute right-0 z-10 shadow-sm border border-gray-200"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7"></path></svg></button>
            </div>
...
<div id="modalTrend" class="fixed inset-0 bg-black/80 z-[60] hidden flex items-center justify-center transition-opacity p-4">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col">
        <div class="bg-black px-6 py-4 flex justify-between items-center"><h3 class="text-white font-bold flex items-center"><svg class="w-5 h-5 text-blue-400 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"></path></svg> Analitik Tren Kinerja</h3><button onclick="closeModal('modalTrend')" class="text-gray-400 hover:text-white"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12"></path></svg></button></div>
...
<div id="modalUrgent" class="fixed inset-0 bg-black/80 z-[60] hidden flex items-center justify-center transition-opacity p-4">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col">
        <div class="bg-red-600 px-6 py-4 flex justify-between items-center"><h3 class="text-white font-bold flex items-center"><svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg> Daftar Tugas Kritis</h3><button onclick="closeModal('modalUrgent')" class="text-red-200 hover:text-white"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12"></path></svg></button></div>
        <div class="p-6"><ul class="space-y-4">
            @forelse($urgentTugas ?? [] as $penugasan)
                @php $sisaHari = \Carbon\Carbon::parse($penugasan->batas_waktu_lapor)->startOfDay()->diffInDays(now()->startOfDay(), false) * -1; @endphp
                <li><a href="{{ url('admin/penugasan/'.($penugasan->id ?? '')) }}" class="{{ $sisaHari < 0 ? 'bg-red-50 border-red-200' : 'bg-yellow-50 border-yellow-200' }} transition p-4 rounded-xl border flex justify-between items-center block cursor-pointer"><div class="flex-grow"><h4 class="font-bold text-gray-900">{{ $penugasan->tugas->nama_tugas ?? '' }}</h4><p class="text-sm text-gray-600 mt-1">Batas: {{ \Carbon\Carbon::parse($penugasan->batas_waktu_lapor)->translatedFormat('d M Y') }}</p></div><span class="px-4 py-2 rounded-lg text-sm {{ $sisaHari < 0 ? 'bg-red-600 text-white' : 'bg-yellow-400 text-gray-900' }} font-bold">{{ $sisaHari < 0 ? 'Terlambat '.floor(abs($sisaHari)).' Hari' : 'Sisa '.floor($sisaHari).' Hari' }}</span></a></li>
            @empty
                <li class="p-6 bg-gray-50 border border-gray-100 rounded-xl text-center text-gray-500 italic">Tidak ada tugas kritis.</li>
            @endforelse
        </ul></div>
    </div>
</div>
...
<div id="modalInbox" class="fixed inset-0 bg-black/80 z-[60] hidden flex items-center justify-center transition-opacity p-4">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col">
        <div class="bg-black px-6 py-4 flex justify-between items-center"><h3 class="text-white font-bold flex items-center"><svg class="w-5 h-5 text-blue-400 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M8 4H6a2 2 0 00-2 2v12a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-2m-4-1v8m0 0l3-3m-3 3L9 8m-5 5h2.586a1 1 0 01.707.293l2.414 2.414a1 1 0 00.707.293h3.172a1 1 0 00.707-.293l2.414-2.414a1 1 0 01.707-.293H20"></path></svg> Log Laporan Terbaru</h3><button onclick="closeModal('modalInbox')" class="text-gray-400 hover:text-white"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12"></path></svg></button></div>
        <div class="p-6"><ul class="space-y-4">
            @forelse($laporanBaru ?? [] as $laporan)
                <li><a href="{{ url('admin/manajemen-laporan/'.($laporan->id ?? '')) }}" class="bg-gray-50 p-4 rounded-xl border flex justify-between items-center hover:bg-blue-50 transition block"><div class="flex-grow"><h4 class="font-bold text-blue-600 text-lg">{{ $laporan->tugas->nama_tugas ?? $laporan->penugasan->tugas->nama_tugas ?? 'Laporan Masuk' }}</h4><p class="text-sm text-gray-600 mt-1">Dikirim oleh: {{ $laporan->user->name ?? 'Pegawai' }}</p></div><span class="px-5 py-2.5 bg-blue-600 text-white rounded-lg font-bold text-sm shadow-md ml-4">Review</span></a></li>
            @empty
                <li class="p-6 bg-gray-50 border border-gray-100 rounded-xl text-center text-gray-500 italic">Belum ada laporan.</li>
            @endforelse
        </ul></div>
    </div>
</div>
...
<script>
    // Menyimpan data dari controller (Seluruh Penugasan Aktif)
    const tasksData = @json(\App\Models\Penugasan::with('tugas')->get() ?? []);
    ...
    // 2. LOGIKA RENDER KALENDER
    const monthNames = ["Januari", "Februari", "Maret", "April", "Mei", "Juni", "Juli", "Agustus", "September", "Oktober", "November", "Desember"];
    const dayNames = ["Min", "Sen", "Sel", "Rab", "Kam", "Jum", "Sab"];

    function getTaskStatus(task, targetDateStr) {
        const target = targetDateStr === 'all' ? new Date() : new Date(targetDateStr);
        target.setHours(0,0,0,0);

        const start = task.tugas && task.tugas.tanggal_mulai ? new Date(task.tugas.tanggal_mulai) : new Date(task.batas_waktu_lapor);
        start.setHours(0,0,0,0);
        const deadline = new Date(task.batas_waktu_lapor);
        deadline.setHours(0,0,0,0);

        const isCompleted = (task.status === 'selesai' || task.status === 'disetujui');

        if (targetDateStr !== 'all' && (target < start || target > deadline)) return null;

        if (isCompleted) return { color: 'gray-800', bgLine: 'bg-gray-800', bgCard: 'bg-gray-50', text: 'SELESAI', labelBg: 'bg-gray-200', labelText: 'text-gray-800' };
        
        const diffDays = Math.ceil((deadline - target) / (1000 * 60 * 60 * 24));
        if (diffDays >= 0 && diffDays <= 2) return { color: 'yellow-400', bgLine: 'bg-yellow-400', bgCard: 'bg-yellow-50', text: 'MENDEKATI BATAS', labelBg: 'bg-yellow-200', labelText: 'text-yellow-800' };
        if (target >= start && target <= deadline) return { color: 'blue-600', bgLine: 'bg-blue-600', bgCard: 'bg-blue-50', text: 'BERJALAN', labelBg: 'bg-blue-200', labelText: 'text-blue-800' };
    ...
    function buildCalendarHtml(dateObj, filterDate, isModal) {
        ...
        for (let i = 1; i <= daysInMonth; i++) {
            ...
            html += `
                <div onclick="${isModal ? 'selectDateModal' : 'selectDate'}('${iterStr}')" class="${btnClass}">
                    <span class="block ${isModal?'text-xs':'text-[10px]'} font-semibold day-text ${dColor}">${dayNames[iterDate.getDay()]}</span>
                    <span class="block ${numSize} font-bold date-text ${tColor}">${i}</span>
                </div>`;
        }
        return html;
    }
    ...
    function goToToday() { currentDate = new Date(); selectedFilterDate = new Date().toISOString().split('T')[0]; renderCalendar(); document.getElementById('dateScrollContainer').scrollLeft = (currentDate.getDate() - 3) * 78; }
    function goToTodayModal() { currentModalDate = new Date(); selectedModalFilterDate = new Date().toISOString().split('T')[0]; renderModalCalendar(); document.getElementById('modalDateScrollContainer').scrollLeft = (currentModalDate.getDate() - 3) * 88; }

    goToToday(); goToTodayModal();

    // 3. LOGIKA MODAL & DRAG SCROLL
    ...
    // 4. CHART.JS
    const cDark = '#111827'; const cBlue = '#2563EB'; const cYellow = '#FACC15'; const cGray = '#9CA3AF';
    const donutData = { labels: ['Selesai', 'Proses', 'Menunggu Review', 'Terlambat', 'Belum Ditugaskan'], datasets: [{ data: [{{ $totalSelesai ?? 0 }}, {{ $tugasAktif ?? 0 }}, {{ $menungguReview ?? 0 }}, {{ $totalTerlambat ?? 0 }}, {{ $belumDitugaskan ?? 0 }}], backgroundColor: [cDark, cBlue, cYellow, '#EF4444', cGray], borderWidth: 2, borderColor: '#fff' }] };
    ...
    const trendData = { labels: {!! json_encode($trendLabels ?? []) !!}, datasets: [ { label: 'Tugas Masuk', data: {!! json_encode($trendMasuk ?? []) !!}, backgroundColor: cDark, borderRadius: 4, barPercentage: 0.6 }, { label: 'Tugas Selesai', data: {!! json_encode($trendSelesai ?? []) !!}, backgroundColor: cBlue, borderRadius: 4, barPercentage: 0.6 } ] };
    const trendOpt = { responsive: true, maintainAspectRatio: false, scales: { x: { grid: { display: false } }, y: { border: { display: false }, grid: { color: '#F3F4F6' } } }, plugins: { legend: { display: false } } };
