...
@section('content')
    ...
    @if (session('success'))
        <div id="toast-success"
            ...
            <button type="button" class="ms-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-lg p-1.5 inline-flex items-center justify-center h-8 w-8" onclick="closeToast()">
    ...
    <div class="p-6">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center mb-6 space-y-4 md:space-y-0">
            ...
            <form action="{{ route('admin.penugasan.index') }}" method="GET" class="flex w-full md:w-auto items-center space-x-2">
            ...
            <div class="flex flex-wrap items-center gap-2">
                <a href="{{ route('admin.penugasan.template') }}" class="bg-emerald-500 text-white px-4 py-2 rounded-lg hover:bg-emerald-600 text-sm flex items-center transition shadow-sm">
                ...
                <a href="{{ route('admin.penugasan.export') }}" class="bg-amber-500 text-white px-4 py-2 rounded-lg hover:bg-amber-600 text-sm flex items-center transition shadow-sm">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1.01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                    Export
                </a>
                ...
                <a href="{{ route('admin.penugasan.create') }}" class="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 text-sm flex items-center transition shadow-sm ml-2">
                ...
                <tbody class="divide-y divide-gray-100 text-gray-700">
                    @forelse ($penugasan as $p)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 font-medium truncate max-w-xs">{{ $p->tugas->nama_tugas ?? 'Tugas Tidak Ada' }}</td>
                            <td class="px-6 py-4 text-sm font-semibold">{{ $p->penerima->name ?? '-' }}</td>
                            <td class="px-6 py-4 text-sm">{{ $p->admin->name ?? '-' }}</td>
                            <td class="px-6 py-4 text-sm">{{ \Carbon\Carbon::parse($p->batas_waktu_lapor)->format('d/m/Y') }}</td>
                            <td class="px-6 py-4">
                                @php
                                ...
                                @endphp
                                @if ($today->gt($batas))
                                    <span class="px-3 py-1 text-xs font-semibold rounded-full bg-red-100 text-red-700">Terlewat</span>
                                @else
                            ...
                            <td class="px-6 py-4">
                                <div class="flex justify-center space-x-3">
                                    {{-- Detail --}}
                                    <a href="{{ route('admin.penugasan.show', $p->id) }}" class="text-blue-500 hover:text-blue-700 transition" title="Detail">
                                    ...
                                    {{-- Edit --}}
                                    <a href="{{ route('admin.penugasan.edit', $p->id) }}" class="text-yellow-500 hover:text-yellow-700 transition" title="Edit">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
