@php
    ...
    $userDict = \App\Models\User::where('role', '!=', 'admin')->get()->keyBy('id');
@endphp

<div id="importModalPenugasan" class="fixed inset-0 z-50 hidden flex items-center justify-center bg-gray-900 bg-opacity-50 transition-opacity">
    <div class="bg-white rounded-lg shadow-xl w-11/12 max-w-6xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div class="p-6 overflow-y-auto flex-grow relative">
            ...
            <div id="dragDropAreaPenugasan" class="border-2 border-dashed border-blue-300 rounded-lg p-12 text-center hover:bg-blue-50 transition cursor-pointer relative">
                <input type="file" id="excelFilePenugasan" accept=".xlsx, .xls" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer">
            ...
            <form id="importFormPenugasan" action="{{ route('admin.penugasan.importProcess') }}" method="POST" class="hidden">
                @csrf
                <div class="mb-4 flex justify-between items-center">
                    <p class="text-sm text-gray-600 font-medium">Preview Data. Pastikan data tugas dan penerima sudah sesuai sebelum di-import.</p>
                    <button type="button" onclick="resetImportPenugasan()" class="text-sm text-red-500 hover:text-red-700 font-medium">Ganti File</button>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 text-sm">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left font-medium text-gray-500">Penerima (ID & Nama)</th>
                ...
                <div class="mt-6 flex justify-end space-x-3 border-t pt-4">
                    <button type="button" onclick="closeImportModal()" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition font-medium">Batal</button>
                    <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition font-medium">Proses Import</button>
                </div>
            </form>
        </div>
    </div>
...
<div id="detailPreviewModal" class="fixed inset-0 z-[60] hidden flex items-center justify-center bg-gray-900 bg-opacity-60 transition-opacity">
    <div class="bg-white rounded-xl shadow-2xl w-11/12 max-w-2xl overflow-hidden border border-gray-200">
        <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-blue-50">
            ...
            <button type="button" onclick="closeDetailModal()" class="text-blue-400 hover:text-blue-700 focus:outline-none">
                <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
            </button>
        </div>
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <div class="space-y-3 text-sm">
                        <div><span class="text-gray-500 block text-xs">Kode Tugas</span><span id="dtlKode" class="font-mono font-semibold text-blue-600"></span></div>
                        <div><span class="text-gray-500 block text-xs">Nama Tugas</span><span id="dtlNamaTugas" class="font-semibold text-gray-800"></span></div>
                        <div><span class="text-gray-500 block text-xs">Keterangan / Deskripsi</span><p id="dtlDeskripsi" class="text-gray-700 mt-1 bg-gray-50 p-2 rounded border border-gray-100 text-xs"></p></div>
                        <div class="grid grid-cols-2 gap-2">
                            <div><span class="text-gray-500 block text-xs">Tgl Mulai</span><span id="dtlTglMulai" class="text-gray-800 font-medium"></span></div>
                            <div><span class="text-gray-500 block text-xs">Tgl Selesai</span><span id="dtlTglSelesai" class="text-gray-800 font-medium"></span></div>
                        </div>
                    </div>
                </div>
                <div>
                    <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Informasi Penerima & Batas</h4>
                    <div class="space-y-3 text-sm">
                        <div><span class="text-gray-500 block text-xs">ID Penerima</span><span id="dtlUserId" class="font-mono text-gray-800"></span></div>
                        <div><span class="text-gray-500 block text-xs">Nama Lengkap</span><span id="dtlUserName" class="font-bold text-gray-800"></span></div>
                        <div><span class="text-gray-500 block text-xs">Role</span><span id="dtlUserRole" class="inline-block bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded text-xs font-semibold capitalize mt-1"></span></div>
                        <div class="mt-4 pt-4 border-t border-gray-100">
                            <span class="text-red-500 block text-xs font-bold mb-1">Batas Waktu Pengumpulan (Lapor)</span>
                            <span id="dtlBatasWaktu" class="text-red-700 font-bold text-lg bg-red-50 px-3 py-1 rounded inline-block border border-red-100"></span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="px-6 py-4 bg-gray-50 border-t border-gray-100 text-right">
            <button type="button" onclick="closeDetailModal()" class="px-4 py-2 bg-gray-800 text-white rounded-lg hover:bg-gray-900 transition font-medium text-sm">Tutup Preview</button>
        </div>
    </div>
</div>
    ...
    function renderPreview(data) {
        ...
        data.forEach((row, index) => {
            ...
            tr.innerHTML = `
                ...
                <td class="px-4 py-2 text-xs text-gray-800">${namaTugas}</td>
                <td class="px-4 py-2">
                    <div class="flex items-center space-x-2">
                        <input type="text" name="penugasan[${index}][id_penerima]" value="${idUser}" readonly class="w-12 text-xs bg-gray-50 border border-gray-200 rounded px-2 py-1 focus:outline-none">
                        <span class="text-xs text-gray-800 font-medium truncate w-32">${namaUser}</span>
                    </div>
                </td>
                <td class="px-4 py-2">
